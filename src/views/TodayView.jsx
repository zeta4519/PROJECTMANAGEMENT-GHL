...
export default function TodayView() {
  ...
  // Tasks due today OR overdue and not completed (carried over from previous days)
  const allTodayTasks = tasks
    .filter(t => t.dueDate && (t.dueDate === today || (t.dueDate < today && t.status !== 'done' && t.status !== 'cancelled')))
    .sort((a, b) => {
  ...
  const overdue = todayTasks.filter(t => t.dueDate < today && t.status !== 'done')
  const pending = todayTasks.filter(t => t.dueDate === today && t.status !== 'done')
  const done = todayTasks.filter(t => t.dueDate === today && t.status === 'done')
  ...
  const totalCount = overdue.length + pending.length + done.length
  const completionPct = totalCount ? Math.round((done.length / totalCount) * 100) : 0
  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 py-6 md:px-8 md:py-8 max-w-2xl w-full mx-auto">
      ...
      <div className="mb-8">
        ...
        {totalCount > 0 && (
          <div className="mt-4 flex items-center gap-3">
            ...
            <span className="text-xs text-white/30">{done.length}/{totalCount} completati</span>
          </div>
        )}
      </div>
      ...
      {overdue.length > 0 && (
        <div className="mb-6">
          <p className="text-xs text-red-400/60 uppercase tracking-wider mb-3 flex items-center gap-1.5">
            <span>⚠</span> In ritardo ({overdue.length})
          </p>
      ...
      {pending.length > 0 && (
        <div className="mb-6">
          {overdue.length > 0 && <p className="text-xs text-white/25 uppercase tracking-wider mb-3">Oggi</p>}
          <div className="flex flex-col gap-2">
